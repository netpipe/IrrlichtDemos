...
// Constructor
//----------------------------------------------------------------------------------------
CTerrain::CTerrain(c8* HeightmapFile, E_TERRAIN_QUALITY Quality, f32 ScaleTexture, scene::ISceneNode* parent, scene::ISceneManager* smgr, s32 id):ISceneNode(parent,smgr,id)
{
	// get heightmap
...
f32 CTerrain::getHeight(f32 x, f32 z)
{
    ...
    if(TempBuffer != NULL)
    {
        ...
        ISceneNode *nodo;
        if (SceneManager->getSceneCollisionManager()->getCollisionPoint(line, selector, intersection,tri,nodo))
        {
