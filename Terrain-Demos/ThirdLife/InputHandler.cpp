...
bool InputHandler::OnEvent(const SEvent& event)
{
	...
	{
			if (Children[i]->OnInputEvent(this, event)) return true;
	}
...
void InputHandler::add(CCompositeInputEventReceiver* newChild)
{
	Children.push_back(newChild);
}

void InputHandler::remove(CCompositeInputEventReceiver* oldChild)
{
	s32 idx = Children.binary_search(oldChild);
	if (idx != -1) Children.erase(idx);
	else printf("InputHandler: Not a child\n");
}