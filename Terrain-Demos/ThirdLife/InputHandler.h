...
struct CCompositeInputEventReceiver;
...
protected:
	irr::core::array<CCompositeInputEventReceiver*> Children;
...
public:
	...
	void add(CCompositeInputEventReceiver* newChild);

	void remove(CCompositeInputEventReceiver* oldChild);
};

struct CCompositeInputEventReceiver : public CCompositeEventReceiver
{
	virtual bool OnEvent(const SEvent& event) {
		printf("CCompositeInputEventReceiver: Use OnInputEvent instead");
		return false;
	}
	virtual bool OnInputEvent(InputHandler* input, const SEvent& event)=0;

	virtual void add(CCompositeInputEventReceiver*) {
		printf("CCompositeInputEventReceiver: Can't add to a leaf\n");
	}

	virtual void remove(CCompositeInputEventReceiver*){
		printf("CCompositeInputEventReceiver: Can't remove from a leaf\n");
	}
};
