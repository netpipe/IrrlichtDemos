...
irrWeatherManager::irrWeatherManager(IrrlichtDevice* device, CPostProcessManager* postProcessManager)
{
    if( !device )
        return;
...
void irrWeatherManager::updateWeather()
{
    if ( timer->getTime() > nextWeatherUpdate )
    {
        ...
        switch ( randInt )
        {
            ...
            case 2:
                sampleEffect = EPPE_SNOW;
                PostProcessManager->render(sampleEffect);
                Cloud->ShowClouds(true, true, true); // 3 clouds shown
            break;
...
E_POSTPROCESS_EFFECT irrWeatherManager::getEPPE() const
{
    return sampleEffect;
}
