...
: Driver(driver), AnimatedMesh(0)
{
	...
	#endif
	MatSystem = Driver->getMaterialSystem();
}
...
bool CMS3DMeshFileLoader::load(io::IReadFile* file)
{
	...
	for (i=0; i<numMaterials; ++i)
	{
		...
		if (TexturePath.trim()!="")
		{
			...
			MaterialPresent = MatSystem->loadMaterial(tmpBuffer->Material,TexturePath.c_str());
			//tmpBuffer->Material.setTexture(0, Driver->getTexture(TexturePath.c_str()) );
		}
		if(!MaterialPresent)
		{
			//this line is completely unessisary materials default to solid. 
