	...
	PrimitivesDrawn(0), TextureCreationFlags(0), AllowZWriteOnTransparent(false)
{
	...
	MaterialSystem = new CMaterialSystem(this,io);
...
void CNullDriver::disableFeature(E_VIDEO_DRIVER_FEATURE feature, bool flag)
{
	...
	MaterialSystem->updateStandardParams();
}
...
IMaterialSystem * CNullDriver::getMaterialSystem()
{
...
s32 CNullDriver::findMaterialRendererId(const c8 *name)
{
	core::stringc sname = name;
	for (u32 i=0; i < MaterialRenderers.size(); ++i)
	{
		if ( sname == MaterialRenderers[i].Name )
			return i;
	}
	return -1;
}
