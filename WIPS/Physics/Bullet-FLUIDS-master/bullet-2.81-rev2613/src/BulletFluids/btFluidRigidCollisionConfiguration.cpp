...
btCollisionAlgorithmCreateFunc* btFluidRigidCollisionConfiguration::getCollisionAlgorithmCreateFunc(int proxyType0, int proxyType1)
{
	...
	//	temporarily use INVALID_SHAPE_PROXYTYPE (replace later with FLUID_SPH_SHAPE_PROXYTYPE)
	...
	if(proxyType0 == INVALID_SHAPE_PROXYTYPE  && collideProxyType1) return m_fluidRigidCreateFunc;
