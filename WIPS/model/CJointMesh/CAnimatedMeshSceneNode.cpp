...
IMesh * CAnimatedMeshSceneNode::getMeshForCurrentFrame()
{
	if (Mesh->getMeshType() == EAMT_SKINNED)
	{
		...
		return skinnedMesh;
#endif
	} else if (Mesh->getMeshType() == EAMT_JOINT)
	{
		//CJointMesh* skinnedMesh = reinterpret_cast<CJointMesh*>(Mesh);

		s32 frameNr = (s32) getFrameNr();
		s32 frameBlend = (s32) (core::fract ( getFrameNr() ) * 1000.f);

		//change state acording to animation
		IMesh *l = Mesh->getMesh(frameNr, frameBlend, StartFrame, EndFrame);

		//update positions of all JointSceneNodes
		for(u32 i = 0; i < JointSceneNodes.size(); ++i)
		{
			JointSceneNodes[i]->UpdateTransformation();
		}
	...
	if (renderMeshes)
	{
		for (u32 i=0; i<m->getMeshBufferCount(); ++i)
		{
			...
			if (transparent == isTransparentPass)
			{
				...
				if (RenderFromIdentity)
					driver->setTransform(video::ETS_WORLD, core::IdentityMatrix );
				else if (Mesh->getMeshType() == EAMT_SKINNED || Mesh->getMeshType() == EAMT_JOINT)
					driver->setTransform(video::ETS_WORLD, AbsoluteTransformation * ((SSkinMeshBuffer*)mb)->Transformation);
...
//! the corresponding joint, if the mesh in this scene node is a Skinned or Joint mesh.
ISceneNode* CAnimatedMeshSceneNode::getJointNode(const core::stringc& jointName)
{
	if (Mesh->getMeshType() == EAMT_SKINNED)
	{
		IBoneSceneNode* bone = getJointNode(jointName.c_str());
		return bone;
	} else if (Mesh->getMeshType() == EAMT_JOINT)
	{
		CJointMesh* jointMesh = reinterpret_cast<CJointMesh*>(Mesh);
		IJointMesh::SJoint *joint = jointMesh->getJoint(jointName);
		CJointSceneNode* n = new CJointSceneNode(this, SceneManager, joint);
		JointSceneNodes.push_back(n);
		return n;
	} else
		return NULL;
}
...
#endif
	else if (Mesh && Mesh->getMeshType() == EAMT_JOINT )
	{
		const f32 frame = getFrameNr(); //old?
		...
		jointMesh->animateMesh(frame);
		core::array<core::matrix4> CurrentState;
		jointMesh->getState(CurrentState);
		...
		if (Transiting != 0.f)
		{
			//TODO Matrix interpolation - CurrentState = matrix4::lerp(PretransitingSave, CurrentState, TransitingBlend);
			for(u32 i = 0; i < CurrentState.size(); i++)
			{
				//Scale is ignored and is mostly ok but if scale key get present we'd need to do
				//scale interpolation too :/

				core::vector3df oldPos = PretransitingSave[i].getTranslation();
				core::vector3df newPos = CurrentState[i].getTranslation();
				core::quaternion oldRot = PretransitingSave[i];
				core::quaternion newRot = CurrentState[i];

				core::quaternion blendedRot;
				core::vector3df  blendedPos;

				blendedRot.slerp(oldRot, newRot, TransitingBlend);
				blendedPos = core::lerp(oldPos, newPos, TransitingBlend);

				CurrentState[i] = blendedRot.getMatrix();
				CurrentState[i].setTranslation(blendedPos);
			}

			jointMesh->setState(CurrentState);
		}
	}
}
...
void CAnimatedMeshSceneNode::beginTransition()
{
	...
	if (TransitionTime != 0)
	{
		...
		} else if (Mesh->getMeshType() == EAMT_JOINT)
		{
			...
			CJointMesh* jointMesh = reinterpret_cast<CJointMesh*>(Mesh);
			jointMesh->getState(PretransitingSave);
		}
