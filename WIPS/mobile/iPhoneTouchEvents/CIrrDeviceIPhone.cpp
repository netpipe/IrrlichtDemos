	...
	, WindowMinimized(false)
{
	...
	TouchControl = new CTouchControl(CreationParams.WindowSize);
...
void CIrrDeviceIPhone::touchEvent(irr::ETOUCH_INPUT_EVENT eventType, s32 x, s32 y, s32 tapCount, f64 timestamp)
{
	irr::SEvent	ievent;
	bzero(&ievent,sizeof(ievent));
	
	ievent.EventType = irr::EET_TOUCH_INPUT_EVENT;
	ievent.TouchInput.Event = eventType;
	
	ievent.TouchInput.X = x;
	ievent.TouchInput.Y = y;
	ievent.TouchInput.TapCount = tapCount;
	ievent.TouchInput.TimeInterval = timestamp;
	
	TouchControl->saveTouchEvent(eventType, x, y, tapCount, timestamp);
