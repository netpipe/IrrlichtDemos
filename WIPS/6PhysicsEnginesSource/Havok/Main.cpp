...
void ThrowBox()
{
	...
	HavokRigidBody *body = Physics->setPhysics(node,10.0);
	...
	body->applyLinearImpulse(hkVector4(newpos[0],newpos[1],newpos[2]));
...
int main(int argc, char **argv)
{
	...
	device->setWindowCaption(L"HavokClass Test Environment");
	...
	for(k = 10; k > 0; k--)
	{
		for(i = 10; i > 0; i--)
		{
			for(j = 1; j > 0; j--)
			{
				node = smgr->addAnimatedMeshSceneNode(smgr->getMesh("Meshes/techcon2.3ds"));
				node->getMaterial(0).setFlag(video::EMF_LIGHTING,false);
				node->setPosition(core::vector3df(-10.0+i*1.0,0.6+k*1,-10.0+j*1.0));
				node->setScale(core::vector3df(0.15,0.15,0.15));
