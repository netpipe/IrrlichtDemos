...
public:
	...
	MyEventReceiver(IrrlichtDevice* device, CFontTool*& fonttool) :
		Device(device), FontTool(fonttool)
	{
...
void createGUI(IrrlichtDevice* device, CFontTool* fc)
{
	...
	*/
	...
	win->setRelativePosition(core::position2di(600-40,200));
}
...
//	CVectorFontTool *vc = 0;
	...
	IEventReceiver *events = new MyEventReceiver(device,fc);
...
/*
			virtual void DrawImage()
			{
				if (FontTool->currentTextures.size() > 0)
					{
						Device->getVideoDriver()->draw2DImage(FontTool->currentTextures[0], core::dimension2d<s32>(10,10));
					}
			};
/**/			
