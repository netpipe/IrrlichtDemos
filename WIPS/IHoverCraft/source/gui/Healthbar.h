...
private:
	...
	irr::video::ITexture* createScaledTexture(irr::video::IImage* origImage, const irr::core::dimension2di& desiredDimension) const
	{
		//make a texture
		...
		irr::video::ITexture* scaledTexture = game->scaleTexture(origTexture,
			irr::core::dimension2di(desiredDimension.Width, desiredDimension.Height));
...
public:
		...
		heartPosition(SX(50), SY(650)), heartTexture(0), heart(0), healtText(0)
	{
		...
		if (heartImage)
		{
			...
			const irr::core::dimension2di desiredDimension(this->iconSize.Width, this->iconSize.Height);
