...
//	std::cin >> C;
#ifdef __EMSCRIPTEN__
	E_DRIVER_TYPE dType = EDT_OGLES2;
	#else
...
//
//	std::cin >> C;
	E_FILTER_TYPE filterType = (E_FILTER_TYPE)core::clamp<u32>((u32)4 - '1', 0, 4);

	// Initialise Irrlicht device.
	device = createDevice(dType, dimension2du(800,600), 32);
	...
	effect->addPostProcessingEffectFromFile(core::stringc("./media/shaders/BrightPass") + shaderExt);
	effect->addPostProcessingEffectFromFile(core::stringc("./media/shaders/BlurHP") + shaderExt);
	effect->addPostProcessingEffectFromFile(core::stringc("./media/shaders/BlurVP") + shaderExt);
	effect->addPostProcessingEffectFromFile(core::stringc("./media/shaders/BloomP") + shaderExt);
