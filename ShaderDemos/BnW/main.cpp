...
int main()
{
  ...
  IMeshSceneNode* cube = smgr->addCubeSceneNode(25);
  cube->setMaterialFlag(EMF_LIGHTING, false);
  cube->setMaterialTexture(0, driver->getTexture("terrain.jpg"));
  ...
  CWaterShader::instance().createMaterial(device);

  // Create water, do not drop()
  // Use remove() to get rid of it
  ...
  while(device->run())
  {
    ...
    if (lastFPS != fps)
    {
      stringw str  = L"Blood & Water - Everything the heart desires :) | ";
              str += driver->getName();
              str += L" | ";
              str += fps;
              str += L" FPS";
