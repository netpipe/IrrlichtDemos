...
void CGUIExtendedText::parseText( const wchar_t* text )
{
    ...
    core::dimension2d<u32> squareDim = CurrentFont->getDimension( L"#" );
    ...
    while ( *c != 0 )
    {
        ...
        if ( str.size() > 0 )
        {
            appendText( str.c_str(), (irr::core::dimension2d<s32>)CurrentFont->getDimension(str.c_str()) );
        }
        ...
        if ( parseToken( c, &str, &tokenParams ) )
        {
            ...
            if ( str == L"" )
            {
                appendText( L"#", (core::dimension2d<s32>)squareDim );
            }
