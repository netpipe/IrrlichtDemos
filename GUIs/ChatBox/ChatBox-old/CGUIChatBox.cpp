...
//
// 08.10.2006 - Updated to irrlicht 1.1 by Chieh Wu (Juliusctw)
//                                          
...
: IGUIChatBox(environment, parent, id, rectangle), ScrollBar(0), 
	ItemHeight(0), TotalItemHeight(0), Font(0), 
	...
	MoveOverSelect(moveOverSelect)
{
	...
	ScrollBar = new FGUIScrollBar(false, Environment, this, 0,
...
bool CGUIChatBox::OnEvent(SEvent event)
{
	...
	case EET_GUI_EVENT:
		...
		case gui::EGET_SCROLL_BAR_CHANGED:
			...
			{
				s32 pos = ((gui::IamarGUIScrollBar*)event.GUIEvent.Caller)->getPos();
				return true;
			}
			break;
...
void CGUIChatBox::draw()
{
	...
	{
		if ( frameRect.UpperLeftCorner.Y + Items[i].height >= AbsoluteRect.UpperLeftCorner.Y &&
			frameRect.UpperLeftCorner.Y <= AbsoluteRect.LowerRightCorner.Y)
		{
			if (Font)
			{
				...
				for (u32 j=0; j < Items[i].BrokenText.size(); ++j)
				{
					...
					r.LowerRightCorner.Y += ItemHeight;
					r.UpperLeftCorner.Y += ItemHeight;
				}
			}
		}
