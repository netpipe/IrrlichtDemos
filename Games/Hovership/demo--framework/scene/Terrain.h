...
private:
	...
	//	mb.Indices.set_free_when_destroyed(false);
		...
		{
			this->vertices[i][0] = mb->getVertexBuffer()[i].Pos.X*scale.X;
			this->vertices[i][1] = mb->getVertexBuffer()[i].Pos.Y*scale.Y;
			this->vertices[i][2] = mb->getVertexBuffer()[i].Pos.Z*scale.Z;
		}

		irr::u32 j;
		for (j = 0; j < mb->getIndexCount(); ++j)
			this->indices[j] = mb->getIndexBuffer()[j];
