...
/*
	This version is altered by Michael Zeilfelder.
	Mainly to get it working with the Android assets system (using the Irrlicht file-system to access that).
*/
...
public:
	...
	*/
	...
	// when to use the Irrlicht filesystem instead of normal files
	enum E_IRR_FS
	{
	...
	void SetIrrFs(irr::io::IFileSystem * fs, E_IRR_FS flags)
	{
		irrFileSystem = fs;
		irrFsFlags = flags;
	}
...
protected :
	// read the xml from data.
	// buffer is null-terminated
	// bufSize contains the size of buffer including the terminating 0
	bool ReadData(const char * buffer, long bufSize, TiXmlEncoding encoding);

	bool LoadFileWithIrrFs(TiXmlEncoding encoding);
