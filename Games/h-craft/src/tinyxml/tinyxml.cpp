...
int TiXmlElement::QueryBoolAttribute( const char* name, bool* _value ) const
{
    if ( !_value)
		return TIXML_WRONG_TYPE;

    int ival = 0;
    int res = QueryIntAttribute( name, &ival );
    *_value = ival ? true : false;
    return res;
}
...
void TiXmlElement::SetBoolAttribute( const char * name, bool value )
{
    SetAttribute( name, value ? 1 : 0 );
}
...
bool TiXmlDocument::LoadFileWithIrrFs(TiXmlEncoding encoding)
{
	if ( !irrFileSystem )
	{
#ifdef _IRR_ANDROID_PLATFORM_
	...
	bool readSuccess = ReadData(buf, length+1, encoding );
...
bool TiXmlDocument::ReadData(const char * buf, long bufSize, TiXmlEncoding encoding)
{
	long length = bufSize - 1;
...
void TiXmlDocument::CopyTo( TiXmlDocument* target ) const
{
	...
	target->irrFileSystem = irrFileSystem;
	target->irrFsFlags = irrFsFlags;
