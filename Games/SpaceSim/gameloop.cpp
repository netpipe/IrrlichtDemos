...
GameLoop::GameLoop(irr::IrrlichtDevice *graphics, KeyListener *receiver):graphics(0),GamePaused(0), pCam(0),then(0),velocity(0)
{
	//set up game
...
//	//this->sound = sound;
	...
	//player create
	CPlayer = new Player(graphics,receiver,core::vector3df(0,0,0));
	...
	//PLACEHOLDER
	CShip *ai = new CShip(graphics, core::vector3df(-5000,0,5000),TERR_PRAETORIAN_CRUISER,FACTION_PROVIAN_CONSORTIUM,TYPE_SHIP);
	Manager->addShip(ai);
	CShip *test = new CShip(graphics,core::vector3df(-2000,0,4000),TERR_PRAETORIAN_CRUISER,FACTION_PROVIAN_CONSORTIUM,TYPE_SHIP);
...
//	//sound->setListenerPosition(pCam->getPosition(),pCam->getRotation());
	...
	//run functions
	playerControl(receiver,frameDeltaTime);					//player key input
	...
	Manager->gameManagerLoop(frameDeltaTime, CPlayer);		//update nodes and game scene ai
...
void GameLoop::playerControl(KeyListener *receiver, f32 frameDeltaTime)
{
