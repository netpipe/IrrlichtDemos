...
#include "StateArenaSelectboard.h"
...
StateArenaSelectboard* StateArenaSelectboard::statearenaselectboard = 0;

StateArenaSelectboard::StateArenaSelectboard() {
	// TODO Auto-generated constructor stub


}

StateArenaSelectboard::StateArenaSelectboard(irr::core::stringc newName) {
	// TODO Auto-generated constructor stub

	StateArenaSelectboard::statearenaselectboard = this;
	...
	//matnr =0;
	//std::cout << "StateArenaSelectboard texturepath FINDME " << matnr << "\n";

	// add billboard
	//
	//	irr::scene::IBillboardSceneNode *node1 = smgr->addBillboardSceneNode ( animModel, irr::core::dimension2d< irr::f32 >(200.0f, 200.0f) );
	//	smgr->addCubeSceneNode(10);
	//	node1->setMaterialTexture(0 ,Mainwindow::device->getVideoDriver()->getTexture("./media/Screens/fondg2.bmp"));
	//	node1->setPosition(irr::core::vector3df(0,100,0));
	//	node1->setMaterialType(irr::video::EMT_SOLID);
	...
	if (drt) {
		...
		animModel->setMaterialTexture(0,
				Mainwindow::device->getVideoDriver()->getTexture(
						"./media/Screens/fondg.bmp"));
		rt = 0;
	}

	// menu logik
	OptionsWidget* tmp;
	tmp = new OptionsWidget("Arena");
	tmp->addoptions("Dragons Lair");

	tmp->addoptions("Lego");
	tmp->addoptions("Origin");
	tmp->addoptions("Pacman");
	tmp->addoptions("Joe");
	tmp->addoptions("Nature");
	tmp->addoptions("Simple");
	tmp->addoptions("Stadium");
	tmp->addoptions("Work");
	tmp->addoptions("Nuclear");
	// add your map here and ..
	//tmp->addoptions("THENAMEOFYOURMAP");
...
std::string StateArenaSelectboard::getValue(std::string aname) {

	std::cout << "StateArenaSelectboard "
			<< entries.getSelectedOptionsWidget()->getValue() << "\n";
	if (entries.getOptionsWidget("Arena")->getValue() == "Dragons Lair") {
		return "./maps/RDragon.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Nuclear") {
		return "./maps/RNuclear.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Lego") {
		return "./maps/RLego.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Origin") {
		return "./maps/ROrigin.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Pacman") {
		return "./maps/RPacman.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Joe") {
		return "./maps/RJoe.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Nature") {
		return "./maps/RClassic.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Simple") {
		return "./maps/RSimple.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Stadium") {
		return "./maps/RStadium.xml";
	}
	if (entries.getOptionsWidget("Arena")->getValue() == "Work") {
		return "./maps/RWork.xml";
	}

	/* (your map) here
	 if (entries.getOptionsWidget("Arena")->getValue() == "THENAMEOFYOURMAP") {
			return "XMLFILEOFYOURMAP";
		}*/
...
void StateArenaSelectboard::render() {
	...
	if (rt) {
		...
		Mainwindow::device->getVideoDriver()->draw2DImage(
				Mainwindow::device->getVideoDriver()->getTexture(
						"./media/Screens/fondg.bmp"), irr::core::position2d<
		...
		font->draw("Select Arena",
				irr::core::rect<irr::s32>(175, 150, 400, 250),
...
void StateArenaSelectboard::updateThisState() {
	statemanager.playerController->update();
	//std::cout << "StateArenaSelectboard lastInterfaceaction "<<Mainwindow::device->getTimer()->getTime() - lastInterfaceAction<<"\n";
	...
	if (Mainwindow::device->getTimer()->getTime() - lastInterfaceAction > 150) {
		...
		//std::cout << "StateArenaSelectboard update \n";
		if (statemanager.playerController->keyReleased(irr::KEY_KEY_S)) {
			//std::cout << "StateArenaSelectboard Pause \n";
		...
		for (int i = 0; i < GameIrrlicht::game->players.size(); i++) {
			...
			if ((GameIrrlicht::game->players[i]->getbutton2()
					...
					|| GameIrrlicht::game->players[i]->getbutton2()
							== PlayerIrrlicht::PRESSED)) {
				std::cout << "StateArenaSelectboard back \n";
...
void StateArenaSelectboard::OnEnter(void) {
	std::cout << "StateArenaSelectboard on enter \n";
	lastInterfaceAction = Mainwindow::device->getTimer()->getTime();
...
void StateArenaSelectboard::OnLeave(void) {
}

//here we get the user input
//this method is derived by IEventReceiver
bool StateArenaSelectboard::OnEvent(const irr::SEvent &event) {
	return true;
}
;
