...
public:
	virtual bool OnEvent(SEvent event) {
		...
		if (event.EventType == irr::EET_MOUSE_INPUT_EVENT && event.MouseInput.Event == EMIE_LMOUSE_PRESSED_DOWN) {
			...
			collision = NewtonCreateBox(nWorld, x, y, z, NULL,offsetMatrix);

			// create the body
			NewtonBody* body;
			 const dFloat* matrix;
			body = NewtonCreateBody(nWorld, collision,matrix);
			tmp->body = body;
...
int main() {
	...
	device = createDevice(EDT_OPENGL, windowSize, windowDepth, bFullscreen, false, true, receiver);
	if (!device) {
		// First try failed. Try again with safer params
		device = createDevice(EDT_OPENGL, dimension2d<u32>(800,600), 16, false, false, true, receiver);
		if (!device) {
			// Exit app
	...
	const dFloat* matrix2;
	NewtonBody* nmapbody = NewtonCreateBody(nWorld, nmapcollision,matrix2);
