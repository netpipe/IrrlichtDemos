...
CIrrOdeGeomTrimesh::CIrrOdeGeomTrimesh(ISceneNode *parent,ISceneManager *mgr,s32 id,
  ...
  if (parent) {
    ...
    while (m_aParams.size()>parent->getMaterialCount()) m_aParams.erase(m_aParams.size()-1);
  }
...
void CIrrOdeGeomTrimesh::initPhysics() {
  ...
  if (getChildren().getSize()) pNode=reinterpret_cast<IMeshSceneNode *>(*(getChildren().begin()));
...
void CIrrOdeGeomTrimesh::setParent(ISceneNode *pParent) {
  ...
  if (pParent) {
    if (!pBody) {
      ...
      while (m_aParams.size()>pParent->getMaterialCount()) m_aParams.erase(m_aParams.size()-1);
    }
    else while (m_aParams.size()>1) m_aParams.erase(m_aParams.size()-1);
  }
}
