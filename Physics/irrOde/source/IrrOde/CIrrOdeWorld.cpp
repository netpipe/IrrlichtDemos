...
void CIrrOdeWorld::initPhysics() {
  ...
  list<CIrrOdeGeom *>::Iterator i,i2;
  ...
  for (i=m_pGeoms.begin(); i!=m_pGeoms.end(); i++) {
    ...
    if (pGeom->isSurfaceMaster()) {
      #ifdef _TRACE_INIT_PHYSICS
        printf("CIrrOdeWorld::initPhysics: \"%s\" surface master of ODE geom class \"%s\"\n",pGeom->getName(),pGeom->getOdeClassname().c_str());
      #endif
      for (i2=m_pGeoms.begin(); i2!=m_pGeoms.end(); i2++)
        if (*i!=*i2) {
          CIrrOdeGeom *g2=(*i2);
          if (pGeom->getOdeClassname()==g2->getOdeClassname()) {
            #ifdef _TRACE_INIT_PHYSICS
              printf("CIrrOdeWorld::initPhysics: setting surface parameters of \"%s\"\n",g2->getName());
            #endif
            pGeom->copyParams(g2,true);
          }
        }
    }
  }
...
void CIrrOdeWorld::removeBody(CIrrOdeBody *pBody) {
  /*s32 idx=m_pBodies.binary_search(pBody);
  if (idx!=-1) m_pBodies.erase(idx);*/
}

void CIrrOdeWorld::removeGeom(CIrrOdeGeom *pGeom) {
  /*s32 idx=m_pGeoms.binary_search(pGeom);
  if (idx!=-1) m_pGeoms.erase(idx);*/
}
...
void CIrrOdeWorld::step(f32 fTime) {
  ...
  m_pOdeDevice->step(fTime,this,m_iJointGroupId);
}
...
void CIrrOdeWorld::quickStep(f32 fTime) {
  ...
  m_pOdeDevice->quickStep(fTime,this,m_iJointGroupId);
}
...
void CIrrOdeWorld::resetBodyTouch() {
  list<CIrrOdeBody *>::Iterator it;

  for (it=m_pBodies.begin(); it!=m_pBodies.end(); it++) (*it)->setIsTouching(NULL);
}

void CIrrOdeWorld::bodyMoved(CIrrOdeEventBodyMoved *pEvent) {
  CIrrOdeBody *pBody=pEvent->getBody();
  if (pBody==NULL) {
    pBody=reinterpret_cast<CIrrOdeBody *>(m_pSceneManager->getSceneNodeFromId(pEvent->getBodyId()));
  }
