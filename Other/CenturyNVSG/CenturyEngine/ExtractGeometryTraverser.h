...
class ExtractGeometryTraverser : public nvtraverser::SharedTraverser
{
  protected:
    ...
    NVSG_API virtual void handleSwitch( const nvsg::Switch *p   //!<  Billboard to handle
                                         );
    ...
    //
    // Override this method to be informed of transformed indexed triangle
    // sets.  Quads will be triangulated before being sumbitted.
    // 
    NVSG_API virtual void submitIndexedTriangleSet( 
                const std::vector<unsigned int> & indices, 
                const std::vector<nvmath::Vec3f> & verts ) = 0;
