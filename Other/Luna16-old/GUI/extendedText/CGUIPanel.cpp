...
bool CGUIPanel::OnEvent( const SEvent& event )
{
    ...
    {
		// Redirecting the event to the scrollbar does not work in 1.4.
		// We will have to scroll manually.
		VScrollBar->setPos(VScrollBar->getPos() + (s32)event.MouseInput.Wheel* -VScrollBar->getSmallStep());
		return true;
	}
