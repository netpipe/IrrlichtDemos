...
function update(msecs)
	...
	local wantedspeed = peak.Vector3D((right - left) * 50, (up - down) * 30, (forward - back) * 50);
	...
	if xrotation * 4 < forward - back then
	...
	if s2var:getInteger() ~= 0 and rocketcooldown > 1000 then
		...
		--rocketscript:callFunctionIII("initialize", body:getPosition().x, body:getPosition().y, body:getPosition().z);
		rocketcooldown = 0;
	end
end
