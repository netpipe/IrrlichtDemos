...
IPostProc::IPostProc( irr::scene::ISceneManager* sm, irr::core::dimension2d<irr::s32> sOut ) {
	smgr = sm;
	init( sOut );
}
IPostProc::IPostProc( IPostProc* follow1, irr::core::dimension2d<irr::s32> sOut ) {
...
void IPostProc::init( irr::core::dimension2d<irr::s32> sOut ) {
	irr::video::IVideoDriver* driver = smgr->getVideoDriver( );
	if( sOut.Width == 0 || sOut.Height == 0 )
		output = driver->addRenderTargetTexture( irr::core::dimension2d<irr::s32>( 1, 1 ) );
	else
	...
	if( supported ) {
		...
		Mesh->Material.TextureLayer[ 0u ].TextureWrap = irr::video::ETC_CLAMP_TO_EDGE;
	}
}
...
void IPostProc::setInput( irr::u8 n, irr::video::ITexture* o ) {
	...
	Tmats[ n ].ZBuffer = 0;
	Tmats[ n ].TextureLayer[ 0u ].TextureWrap = irr::video::ETC_CLAMP_TO_EDGE;
