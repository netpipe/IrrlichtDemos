...
CLensFlarePostProc::CLensFlarePostProc( IPostProc* follow, irr::core::dimension2d<irr::s32> sOut, irr::video::ITexture* m, irr::core::vector3d<irr::f32> p, irr::f32 r ) : IPostProc( follow, sOut ) {
	setPosition( p, r );
	init( follow, sOut, m );
}
CLensFlarePostProc::CLensFlarePostProc( IPostProc* follow, irr::core::dimension2d<irr::s32> sOut, irr::video::ITexture* m, irr::scene::ISceneNode* p, irr::f32 r ) : IPostProc( follow, sOut ) {
	setPosition( p, r );
	init( follow, sOut, m );
}
...
void CLensFlarePostProc::init( IPostProc* follow, irr::core::dimension2d<irr::s32> sOut, irr::video::ITexture* m ) {
...
//	flT.push_back( irr::core::rect<irr::f32>( 0.75f, 0.0f, 1.0f, 0.25f ) );
	...
	if( supported ) {
		...
		mf->Material.TextureLayer[ 0u ].TextureWrap = irr::video::ETC_CLAMP_TO_EDGE;
	}
	setInput( 0u, follow );
}
void CLensFlarePostProc::renderEffect( irr::video::ITexture* target ) {
	...
	irr::core::dimension2d<irr::s32> dsc = driver->getScreenSize( );
	if( pS.X < 0 || pS.Y < 0 || pS.X >= dsc.Width || pS.Y >= dsc.Height )
		return;
	
	irr::core::dimension2d<irr::s32> dim = tex->getSize( );
