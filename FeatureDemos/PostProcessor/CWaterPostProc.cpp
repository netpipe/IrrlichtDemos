...
CWaterPostProc::CWaterPostProc( IPostProc* follow, irr::core::dimension2d<irr::s32> sOut, irr::f32 depth ) : IPostProc( follow, sOut ) {
	...
	if( supported ) {
		...
		matUnder.ZBuffer = 0;
		matUnder.TextureLayer[ 0u ].TextureWrap = irr::video::ETC_CLAMP_TO_EDGE;
	}
	setInput( 0u, follow );
}
