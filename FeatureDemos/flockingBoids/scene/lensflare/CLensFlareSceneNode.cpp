...
/*
irr::u32 CLensFlareSceneNode::getMaterialCount() const
{
	return 1;
}
*/
...
void CLensFlareSceneNode::OnRegisterSceneNode()
{
	if(IsVisible)
	{
		const core::position2d<s32> pm = SceneManager->getSceneCollisionManager()->getScreenCoordinatesFrom3DPosition(
...
void CLensFlareSceneNode::render()
{
	...
	const core::dimension2di& sz = SceneManager->getVideoDriver()->getScreenSize();
	const core::vector2df& mid = core::vector2df((f32)sz.Width, (f32)sz.Height) * 0.5f;

	const core::position2di& lp = SceneManager->getSceneCollisionManager()->getScreenCoordinatesFrom3DPosition(getAbsolutePosition(), camera);
