...
public:
    ...
    CLoadingBar(IrrlichtDevice* in_d, video::IVideoDriver* in_driver, IGUIEnvironment* in_guienv, ITexture* in_backgroundImageTexture, const rect<s32>& rectangle, bool vis,s32 TotalNum)
    :device(in_d), Percent(0), Text(true), DisplayText(L""), Border(true), isVisiblee(vis), AbsoluteBorderRect(rectangle), BorderRect(rectangle), MaxNum(TotalNum),isDone(false){
